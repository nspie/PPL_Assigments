...
import { applyEnv, applyEnvBdg, globalEnvAddBinding, makeExtEnv, setFBinding,
            theGlobalEnv, Env, FBinding, setBox, makeBox, unbox } from "./L4-env-box";
...
const evalTraceExp = (exp: TraceExp): Result<void> => {
    let result = applyEnvBdg(theGlobalEnv, exp.var.var); // < OK, FBinding("", Box) >
    if(result.tag === "Ok") 
        return makeOk(setBox(result.value.val, makeTracedClosure((unbox(result.value.val)) as Closure, exp.var.var)))
    else
        return makeFailure<void>("");
}
...
let counterBox = makeBox(0);

const applyTracedClosure = (proc: TracedClosure, args: Value[]): Result<Value> => {

    printPreTrace(proc.name, args, unbox(counterBox));
    setBox(counterBox, unbox(counterBox) + 1);

    const vars = map((v: VarDecl) => v.var, proc.closure.params);
    let ans = evalSequence(proc.closure.body, makeExtEnv(vars, args, proc.closure.env));

    setBox(counterBox, unbox(counterBox) - 1);

    if (ans.tag === "Ok") printPostTrace(ans.value, unbox(counterBox));
